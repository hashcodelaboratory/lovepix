...
const create = async (req: NextApiRequest, res: NextApiResponse<unknown>) => {
  try {
    ...
    const config = {
      headers: {
        Authorization: `SFAPI email=${process.env.AUTH_EMAIL}&apikey=${process.env.AUTH_API_KEY}&company_id=${process.env.AUTH_COMPANY_ID}&module=${process.env.AUTH_MODULE}`,
