...
const Price = () => {
  ...
  const { width, height } = DIMENSIONS.find((dim) => dim.id === configuration?.dimensionId) ?? { width: 0, height: 0 };

  const price = width > 0 && height > 0 ? getPrice(width, height, materials.find(material => material.id === configuration?.material)?.name) : "-";
