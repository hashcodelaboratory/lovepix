...
describe('pages/api/smart-emailing/status.ts', () => {
  ...
  describe('status', () => {
    it('should integrate `statusEndpoint` invocation', async () => {
      // @ts-ignore
      await status({} as NextApiRequest, {status: jest.fn().mockReturnValue({json: jest.fn()})} as NextApiResponse)
    ...
    it('should integrate `authorizationHeaders` invocation', async () => {
      (statusEndpoint as jest.Mock).mockReturnValue({href: '<status_endpoint>'})
      // @ts-ignore
      await status({} as NextApiRequest, {status: jest.fn().mockReturnValue({json: jest.fn()})} as NextApiResponse)
    ...
    it('should log request trigger with expected information', async () => {
      (statusEndpoint as jest.Mock).mockReturnValue({href: '<status_endpoint>'})
      // @ts-ignore
      await status({method: 'GET'} as NextApiRequest, {status: jest.fn().mockReturnValue({json: jest.fn()})} as NextApiResponse)
    ...
    it('should invoke `axios` get with expected request composition', async () => {
      ...
      expect(axios.get).toHaveBeenCalledWith("<status_endpoint>", {headers: {Authorization: '<authorization_token>'}})
    })
    ...
    it('should return 200 with expected request result data', async () => {
      ...
      // @ts-ignore
      await status({} as NextApiRequest, {status: (statusCode) => mockedStatusInvocation(statusCode)} as NextApiResponse)
    ...
    describe('when there is some thrown exception', () => {
      it('should return 500 with expected failure status', async () => {
        ...
        expect(mockedJson).toHaveBeenCalledTimes(1)
        expect(mockedJson).toHaveBeenCalledWith({message: "<test_error_message>", status: "NOK"})
      })
    })
  })
})